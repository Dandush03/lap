...
RSpec.describe 'Api::Admin::Auth::Sessions', type: :request do
  ...
  path '/api/admin/auth/admins/sign_in' do
    ...
    get 'Retrieves CSRF Token And User' do
      ...
      response '202', 'Fetch CSRF Token and User if Already Login' do
        before do
          post '/api/admin/auth/admins/sign_in', params: {
    ...
    post 'Login User' do
      ...
      parameter name: :api_admin_auth_admin, in: :formData, schema: {
        type: :object,
        required: %i[api_admin_auth_admin[login] api_admin_auth_admin[password]],
        properties: {
      ...
      response '201', 'Login User' do
        let(:api_admin_auth_admin) do
          {
            'api_admin_auth_admin[login]': admin.username,
            'api_admin_auth_admin[password]': '123123'
          }
        end
        run_test!
      end
